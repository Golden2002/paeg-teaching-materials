# -*- coding: utf-8 -*-
"""paeg_teaching_materials.llm_client —— DeepSeek LLM 客户端（OpenAI 兼容，零第三方依赖）。

让教学物料工具【独立接入 LLM】：不依赖宿主注入，直接读环境变量即可调用。

Key 读取顺序（与主项目 / 词汇表 / 简历工具同一套约定）：
  1. 环境变量 DEEPSEEK_API_KEY（最优先）
...
无 key 时 __call__ 返回空串（等价 NullLLM），生成器降级到弱模式——不抛异常。
"""
